...
import { BookOpen, Briefcase, Clock, Award, Clapperboard } from 'lucide-react';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            const [books, companies, projects, expertise, ceiaProjects] = await Promise.all([
                supabase.from('books').select('*', { count: 'exact', head: true }),
                supabase.from('companies').select('*', { count: 'exact', head: true }),
                supabase.from('projects').select('*', { count: 'exact', head: true }),
                supabase.from('expertise').select('*', { count: 'exact', head: true }),
                supabase.from('ceia_projects').select('*', { count: 'exact', head: true }),
            ]);
            ...
            setStats({
                ...
                ceiaProjects: ceiaProjects.count || 0
            });
        };
        fetchStats();
    }, []);
    ...
    const cards = [
        ...
        { title: 'Projetos CEIA', count: stats.ceiaProjects, icon: <Clapperboard className="text-indigo-500" />, color: '#6366f1' },
